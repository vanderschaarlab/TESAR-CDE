...
def generate_plot_gamma(x, y, y_std, names, xlim, filename):
    ...
    ax.set_ylim([0, None])
    leg = ax.legend(
        fontsize=12, frameon=False, facecolor="gray", framealpha=0.05, edgecolor="black"
    )
...
rmses_avg_std = np.array(
    [rmse_avg_baseline_std, rmse_avg_two_step_std, rmse_avg_multitask_std]
)
...
rmses_tau1_std = np.array(
    [rmse_tau1_baseline_std, rmse_tau1_two_step_std, rmse_tau1_multitask_std]
)
...
rmses_gamma6 = np.array(
    [rmse_gamma6_baseline, rmse_gamma6_two_step, rmse_gamma6_multitask]
)
...
rmses_gamma6_std = np.array(
    [rmse_gamma6_baseline_std, rmse_gamma6_two_step_std, rmse_gamma6_multitask_std]
)
...
for i in range(len(rmses_gamma6)):
    ...
    ax2.fill_between(
        taus,
        rmses_gamma6[i, :] - rmses_gamma6_std[i, :],
        rmses_gamma6[i, :] + rmses_gamma6_std[i, :],
        alpha=0.1,
        color=colors[i],
        edgecolor=None,
...
rmses_intcovAvg = np.array(
    [rmse_intcovAvg_baseline, rmse_intcovAvg_two_step, rmse_intcovAvg_multitask]
)

rmse_intcovAvg_baseline_std = [
    0.009455,
    0.009453,
    0.009484,
    0.009493,
...
rmses_intcovAvg_std = np.array(
    [
        rmse_intcovAvg_baseline_std,
        rmse_intcovAvg_two_step_std,
        rmse_intcovAvg_multitask_std,
    ]
)
...
for i in range(len(rmses_intcovAvg)):
    ...
    plt.fill_between(
        gammas[0:-1],
        rmses_intcovAvg[i, :] - rmses_intcovAvg_std[i, :],
        rmses_intcovAvg[i, :] + rmses_intcovAvg_std[i, :],
        alpha=0.1,
        color=colors[i],
        edgecolor=None,
...
rmses_maxInt1_2 = np.array(
    [rmse_maxInt1_baseline_2, rmse_maxInt1_two_step_2, rmse_maxInt1_multitask_2]
)
...
rmses_maxInt1_std_2 = np.array(
    [
        rmse_maxInt1_baseline_std_2,
        rmse_maxInt1_two_step_std_2,
        rmse_maxInt1_multitask_std_2,
    ]
)
...
for i in range(len(rmses_maxInt1_2)):
    ...
    plt.fill_between(
        scaling_factors,
        rmses_maxInt1_2[i, :] - rmses_maxInt1_std_2[i, :],
        rmses_maxInt1_2[i, :] + rmses_maxInt1_std_2[i, :],
        alpha=0.1,
        color=colors[i],
        edgecolor=None,
