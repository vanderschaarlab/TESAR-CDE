...
def sim_hawkes(states, kappa, intensities=None):
    ...
    i = 0
    while done == False:
        overall_timestamps = get_timestamps(
            states, kappa, process="Poisson", intensities=intensities
        )
...
def data_sampler(
    ...
    for idx, indiv_stage in enumerate(cancer_stages):
        ...
        final_hawkes.extend(
            sim_hawkes(
                indiv_stage, kappa, intensities=raw_data["obs_probabilities"][idx]
            )
        )
...
def transform_data(
    ...
    data["test_data_factuals"]["obs_probabilities"] = np.ones_like(
        data["test_data_factuals"]["obs_probabilities"]
    )
    data["test_data"]["obs_probabilities"] = np.ones_like(
        data["test_data"]["obs_probabilities"]
    )
    # "Sample"
    ...
    updated_data["test_data_factuals"][
        "obs_probabilities"
    ] = obs_prob_hypothetical_factual
